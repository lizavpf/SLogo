				...
				if (program != null) {      
				    ...
				    try {
						...
						while ((currLine = reader.readLine()) != null) {
							instructions.append(currLine.toUpperCase());
						}
						
						reader.close();
						
						String finalInstructions = instructions.toString();
						
						StringReader newReader = new StringReader(finalInstructions);
						
						SLogoInterpreter interpreter = new SLogoInterpreter();
						
						List<String> evaluateList = interpreter.interpret(newReader, turtleContext);
